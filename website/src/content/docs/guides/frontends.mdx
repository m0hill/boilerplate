...
`Cloudflare.Website.Vite` covers TanStack Start, Astro, SolidStart, Nuxt, and
plain React. `Cloudflare.Website.StaticSite` wraps any custom build pipeline
— Hugo, Eleventy, mdBook, Jekyll. One declaration. Alchemy builds
and deploys to Cloudflare.

## Vite-based frameworks

`Cloudflare.Website.Vite` uses Cloudflare's Vite plugin to build the server
...
For everything Vite doesn't build, `Cloudflare.Website.StaticSite` runs a
...
export const Blog = Cloudflare.Website.StaticSite("Blog", {
  command: "hugo --minify",
  outdir: "public",
});
```
