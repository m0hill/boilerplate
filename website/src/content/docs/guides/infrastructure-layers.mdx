...
export const JobServiceR2 = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const r2 = yield* Cloudflare.R2.ReadWrite(Bucket);
...
contract — internally it yields an `Bucket` resource and binds
...
- [Binding](/concepts/binding) — what `Binding(resource)` does under the hood
