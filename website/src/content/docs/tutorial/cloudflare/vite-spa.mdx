...
`Cloudflare.Website.Vite` invokes Vite programmatically (via
...
For a `Cloudflare.Website.Vite` worker that means Alchemy boots Vite's dev
...
`Cloudflare.Website.Vite` works with any Vite-based framework. The setup
above is the SPA case (no SSR). For SSR frameworks, drop the
`assets` block and let the framework's Vite plugin own the
...
- **Plain static site** — a single `index.html` (no framework)
  ships through `Cloudflare.Website.Vite("Website")` with no extra config.
