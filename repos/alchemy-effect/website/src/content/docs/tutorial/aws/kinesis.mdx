...
`Kinesis.consumeStreamRecords(stream, ...)` mirrors the SQS
`messages(...)` shape from the previous part.
...
resources its `bind`/`messages`/`process` calls touched — no
...
a matter of declaring the resource and pointing `messages` /
`.process` at it.
