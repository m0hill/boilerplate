...
description: Replace alchemy's built-in DO method bridge with `Cloudflare.RpcDurableObject` — a typed RPC group served on the DO's `fetch`, with a class-instance-preserving wire format. Walk through a single DO, drive it from a test, then layer in a Worker.
sidebar:
  order: 12
---

`Cloudflare.RpcDurableObject` lets you define a Durable
Object's API as an [Effect RPC](/guides/effect-rpc) group — the same
schema-typed `RpcGroup` you'd use anywhere else in Effect, served on
the DO's `fetch`.
...
uses alchemy's built-in method bridge, which `JSON.stringify`s each
return value and strips class identity. `RpcDurableObject`
runs an Effect RPC server on the DO's `fetch` instead, so both ends
share one codec.
:::
...
`Cloudflare.RpcDurableObject<Self>()(...)` mirrors
`Cloudflare.DurableObject<Self>()(...)` — same outer/inner
...
but its methods — like `storage.put` / `storage.get` — are
[colored with `RuntimeContext`](/concepts/layers#runtime-as-a-colored-function),
so you can only *use* them in the inner (runtime) Effect. That's why
`state` is yielded in the outer block but the handler closures that
call `state.storage` live in the inner one.
:::
...
Switch to `RpcDurableObject` when:
...
Mirrors `yield* DurableObject` on the regular class — useful
...
- `Cloudflare.RpcDurableObject<Self>()(name, { schema }, impl)`
