...
description: Receive messages from a Cloudflare Queue with Cloudflare.Queues.consumeQueueMessages(queue, handler) — Effect-style stream handler with automatic ack/retry.
sidebar:
  order: 10
---
...
`Cloudflare.Queues.WriteQueue(...)` from the [Queue producer
...
- Cloudflare.Queues.messages<QueueMessageBody>(queueResource, (stream) =>
+ Cloudflare.Queues.messages<QueueMessageBody>(
+   queueResource,
+   {
+     batchSize: 25,
+     maxRetries: 3,
