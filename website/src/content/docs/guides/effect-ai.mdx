...
For Workers AI on Cloudflare, `Cloudflare.AI.Gateway` declares an AI
Gateway as a resource and `Cloudflare.AI.QueryGateway(Gateway)` returns
a typed client whose `.model({...})` builds a `LanguageModel` Layer
that proxies Workers AI through the gateway — with caching, rate
limiting, retries, and a unified request log.
...
- `Cloudflare.AI.Gateway` (Workers AI, via Alchemy itself)
