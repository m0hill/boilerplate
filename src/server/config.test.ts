...
describe("server configuration", () => {
  it("uses the approved defaults", async () => {
    await expect(loadConfig({})).resolves.toEqual({ host: "0.0.0.0", port: 3000 })
  })

  it("loads HOST and PORT", async () => {
    await expect(loadConfig({ HOST: "127.0.0.1", PORT: "8080" })).resolves.toEqual({
      host: "127.0.0.1",
      port: 8080,
    })
  })
